...
func (m *Manager) initChannels() error {
	...
	if m.config.Channels.Bitrix24.Enabled && m.config.Channels.Bitrix24.Domain != "" {
		logger.DebugC("channels", "Attempting to initialize Bitrix24 channel")
		bitrix24, err := NewBitrix24Channel(m.config.Channels.Bitrix24, m.bus)
		if err != nil {
			logger.ErrorCF("channels", "Failed to initialize Bitrix24 channel", map[string]interface{}{
				"error": err.Error(),
			})
		} else {
			m.channels["bitrix24"] = bitrix24
			logger.InfoC("channels", "Bitrix24 channel enabled successfully")
		}
	}
